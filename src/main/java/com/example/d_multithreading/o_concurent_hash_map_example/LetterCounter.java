...
public abstract class LetterCounter {
    ...
    protected LetterCounter(final int subtaskCount) {
    ...
    private Stream<Subtask> createSubtasks(
            final Map<Character, Integer> accumulator, final String input
    ) {
        final int subtaskCharCount = findSubtaskCharCount(input);
        return range(0, subtaskCharCount).mapToObj(i -> createSubtask(
                accumulator, input, subtaskCharCount, i
        ));
    }
