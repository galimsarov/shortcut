...
`Stream API` — **декларативная обработка последовательности данных** через pipeline-операции
...
### Анонимный класс — одноразовая inline-реализация класса или интерфейса без явного имени

### компактная форма передачи поведения (реализация функционального интерфейса)
